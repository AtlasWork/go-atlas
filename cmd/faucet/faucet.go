...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Atlas protocol")
	statsFlag   = flag.String("ethstats", "", "AtlasStats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Atlas and the faucet")
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s WORKs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
func authGooglePlus(url string) (string, string, common.Address, error) {
	...
	address := common.HexToAddress(string(regexp.MustCompile("0x[0-9a-fA-F]{40}").Find(body)))
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Atlas address found to fund")
	}
	var avatar string
