...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Atlas protocol")
	...
	log.Info("Light Atls protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Atlas peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Atlas message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Atlas message arrived", "code", msg.Code, "bytes", msg.Size)
